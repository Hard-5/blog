...
| 全文搜索 | 标题 + 摘要 + 标签 + 正文，边打边筛，命中处高亮；按 `/` 聚焦，`Esc` 清空 |
| 标签系统 | 首页直接按标签筛选 + 标签总览页 |
| 归档页 | 按年份分组，显示每年的篇数与字数 |
| 文章目录 | 自动从 `##` / `###` 生成，滚动时高亮当前小节；手机上收成可点开的折叠条 |
| 代码高亮 | 自己实现的轻量高亮，支持 JS/TS、Python、C/C++、Java、Go、Rust、Bash、SQL、YAML 等 |
| 复制代码 | 每个代码块右上角一键复制 |
| 分享 | 文章页「分享」按钮复制固定链接；每篇文章都有独立静态页，微信/QQ 分享卡片正常显示标题和摘要 |
| SEO | 每篇文章生成 `sitemap.xml`、`robots.txt`、Open Graph 标签和结构化数据，百度能收录正文 |
| 无 JS 可读 | 导航和文章列表静态注入首页，关掉 JavaScript 照样能读完全站 |
| 响应式 | 手机上自动变单栏，目录折叠 |
| 阅读进度条 | 文章页顶部 |
| 上下篇导航 | 自动找上一篇 / 下一篇 |
| 无障碍 | 跳转正文链接、键盘焦点可见 |
...
```text
blog/
├── index.html              # 首页（构建时会把导航和文章列表静态注入进去）
├── site.json               # 站点名称、简介、导航、社交链接
├── package.json            # 只给命令起别名，没有任何依赖
├── posts/                  # 文章（Markdown，这是你唯一需要天天动的地方）
├── pages/about.md          # 「关于」页面
├── p/                      # 【构建产物】每篇文章的独立静态页，用于分享和 SEO
├── feed.xml                # 【构建产物】RSS 订阅源
├── sitemap.xml             # 【构建产物】给搜索引擎的页面清单
├── robots.txt              # 【构建产物】爬虫规则
├── assets/
│   ├── css/style.css       # 全部样式 + 深浅色主题变量
│   ├── img/                # 图片
│   └── js/
│       ├── markdown.js     # Markdown 渲染器 + 代码高亮（构建期与运行时共用）
│       ├── ui.js           # 主题 / 复制 / 目录 / 进度条（首页和静态页共用）
│       ├── app.js          # 首页的路由、搜索、标签、归档
│       └── posts-data.js   # 【构建产物】文章数据
└── tools/
    ├── build.mjs           # Markdown → posts-data.js + p/*.html + sitemap + RSS
    ├── new-post.mjs        # 新建文章
    ├── check.mjs           # 产物自检（含静态页、SEO、隐私词）
    ├── smoke.mjs           # 无浏览器冒烟测试
    ├── deploy.mjs          # 一条命令发布到 GitHub Pages
    └── serve.mjs           # 本地预览服务器
```

`p/`、`feed.xml`、`sitemap.xml`、`robots.txt`、`posts-data.js` 都是构建产物，
**每次 `npm run build` 会重新生成，不要手改**。

### 为什么要生成 `p/*.html`

首页是 JavaScript 渲染的，而**搜索引擎（尤其百度）和微信/QQ 的分享卡片抓取器都不执行 JavaScript**。
如果只有首页这一张 JS 空壳，结果是：百度搜不到你的任何一篇文章，分享出去的卡片永远是同一句站点简介。

所以构建时会为每篇文章额外生成一份纯静态 HTML，里面包含完整正文、Open Graph 标签和结构化数据。
它带来三个好处：搜索引擎能收录、分享到微信/QQ 能正确显示标题和摘要、关掉 JS 也能读。
